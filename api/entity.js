// Server-rendered Zoi entity page: full HTML + schema.org JSON-LD for search + AI indexing.
...
function jsonld(e,url){
  ...
  if(e.website) o.sameAs=[e.website];
  if(e.address||e.city){
  ...
  if(e.website) o.url=url; // canonical points to Zoi page; website in sameAs
  if(e.price_range) o.priceRange=e.price_range;
  if(e.rating!=null && e.rating_count!=null && e.rating_count>0){
    o.aggregateRating={ '@type':'AggregateRating', ratingValue:e.rating, reviewCount:e.rating_count };
  }
  var sl=e.social_links||{}; var sameAs=[]; if(e.website) sameAs.push(e.website);
  ['instagram','facebook','tiktok','youtube','twitter','linkedin'].forEach(function(k){ if(sl[k]) sameAs.push(sl[k]); });
...
function page(e){
  ...
  return '<!doctype html><html lang="en"><head><meta charset="utf-8">'
   ...
   +'.badge{display:inline-block;font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:#8a6a24;font-weight:700}h1{font-size:30px;margin:6px 0 4px;line-height:1.15}'
   ...
   +'p.desc{font-size:17px}footer{margin-top:34px;font-size:13px;color:#5c7b92}</style></head><body><div class="wrap">'
